...
Builder::Builder(const QString &table, const QString connecName):
    tableClause(table),
    connectionName(connecName),
    limit(0),
    offset(0)
{
...
Collection Builder::select(const QString &column) const
{
...
Model Builder::first()
{
    Collection collection = take(1).select();
    return collection.size() ? collection.first() : Model();
}

Builder::Query Builder::exec(const QString &statement, QSqlDatabase db)
{
    return Query(statement, db);
}
    ...
    succeeded(false)
{
    ...
    qDebug()<<QString("[SqlQuery] %1 ms, %2").arg(t.elapsed(), 2).arg(statement);
}

Builder::Query::Query(const QString &statement, QSqlDatabase db) :
    QSqlQuery(db),
    succeeded(false)
{
    QTime t;
